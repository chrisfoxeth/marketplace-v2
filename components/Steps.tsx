        ...
        <div className="mb-5 flex gap-3" key={action + index}>
          <div className="h-10 w-10">
            {error ? (
              ...
              <div className="reservoir-h6 font-thin mr-1 ml-1 flex h-8 w-8 items-center justify-center rounded-full text-center font-headings ring-2 ring-inset ring-neutral-900 dark:text-white  dark:ring-white">
                <div>{index + 1}</div>
              </div>
            )}
          </div>
          <div>
            <div className="reservoir-h6 font-thin mb-1 mt-0.5 font-headings dark:text-white">
              {action}
            </div>
            {error && (
              <div className="reservoir-h6 font-thin mb-2.5 font-headings text-red-800 dark:text-white">
                {error}
              </div>
            )}
            {firstIncompleteStepIndex === index && (
